...
export const PosPurchaseHistory = () => {
  ...
  const data = useSelector(fetchUserOrders);
  const userId = useSelector(fetchUser);
  const userStatus = useSelector((state) => state.user.status);

  const handleOnClick = (row) => navigate(`${pathname}/${row.original.id}`);

  useEffect(() => {
    userStatus === "idle" && dispatch(fetchUserOrders(userId));
  }, [dispatch, userId, userStatus]);

  useEffect(() => {
    dispatch(fetchAnOrder(orderId));
  }, [dispatch, orderId]);
          ...
          <div className="flex items-center space-x-3">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">
                {userId != null ? user.name : "No Records"}
              </h1>
            </div>
          </div>
        </div>
